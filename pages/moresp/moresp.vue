	...
	<view class="page">
		<view class="sachv">
			...
			<image class="icon" src="../../static/index/sach.png" mode=""  @click="gosach"  ></image>
						...
						<input v-model="name"  placeholder="视频名" @confirm="gosach" type="text">\
				...
				<view class="box" v-if="data.length!=0">
					...
					<view class="item" v-for="(i,index) in data" @click="gospbf(i.videoId)">
						<text>{{index+1}}</text>
...
<script>
	...
	import {muiscapi,userapi,userxx,usermusicis,usershopmusic,usersp,namespss} from "../../api/index.js"
	export default {
		data() {
			return {
				name:"",
		...
		methods: {
			async gosach(){
				this.data=(await namespss(this.name)).rows
			},
			settab(i){
				this.istab=i
			},
